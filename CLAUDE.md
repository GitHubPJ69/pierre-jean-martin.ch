...
- **Never put the email address as a literal string in HTML.** It's assembled at runtime in `app.js` (`['contact','pierre-jean-martin.com'].join('@')`) and injected into `[data-email]` elements.
...
- **Email**: never written literally; only changed inside `injectEmail()` in `app.js`.
